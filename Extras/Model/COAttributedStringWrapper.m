...
- (void)registerToObserveBacking
{
    ...
    for (COAttributedStringChunk *chunk in _backing.chunks)
    {
        ...
        [chunk addObserver: self
                forKeyPath: @"text"
                   options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld
                   context: NULL];
        [chunk addObserver: self
                forKeyPath: @"attributes"
                   options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld
                   context: NULL];
    }
...
static void LengthOfCommonPrefixAndSuffix(NSString *a,
                                          NSString *b,
                                          NSUInteger *prefixOut,
                                          NSUInteger *suffixOut)
{
                       ...
                       userInfo: (id)info
{
  ...
  changeInLength: -deletedChunksLength];
}
...
- (NSDictionary *)attributesAtIndex: (NSUInteger)anIndex effectiveRange: (NSRangePointer)aRangeOut
{
    ...
    COAttributedStringChunk *target = [self.backing chunkContainingIndex: anIndex
...
- (void)replaceCharactersInRange: (NSRange)aRange withString: (NSString *)aString
{
    ...
    NSUInteger chunkIndex = 0, chunkStart = 0;
    COAttributedStringChunk *chunk = [_backing chunkContainingIndex: aRange.location
                                                         chunkStart: &chunkStart
                                                         chunkIndex: &chunkIndex];
    ...
    if (chunk == nil && aRange.location == self.length)
    {
        ...
        chunk = [_backing chunkContainingIndex: aRange.location - 1
                                    chunkStart: &chunkStart
                                    chunkIndex: &chunkIndex];
    }
    ...
    NSString *newText = [chunk.text stringByReplacingCharactersInRange: NSMakeRange(indexInChunk,
    ...
    while (remainingLengthToDelete > 0)
    {
        ...
        chunk.text = [chunk.text stringByReplacingCharactersInRange: NSMakeRange(0,
                                                                                 lengthInChunkToReplace)
                                                         withString: @""];
...
- (void)setAttributes: (NSDictionary *)aDict range: (NSRange)aRange
{
    ...
    {
        NSUInteger chunkIndex = 0, chunkStart = 0;
        COAttributedStringChunk *target = [self.backing chunkContainingIndex: aRange.location
                                                                  chunkStart: &chunkStart
                                                                  chunkIndex: &chunkIndex];
            ...
            && (chunkStart + target.length) >= NSMaxRange(aRange))
        {
            ...
            if ([COAttributedStringAttribute isAttributeSet: existingAttribs
                                                 equalToSet: proposedAttribs])
            {
...
- (void)endEditing
{
    ...
    if (reachedZero)
    {
        ...
        {
            [NSException raise: NSInternalInconsistencyException
                        format: @"COAttributedStringWrapper user made incorrect calls to -edited:range:changeInLength:. Expected length %d, actual length at end of batch is %d",
                                (int)expectedLength,
                                (int)lengthAtEndOfBatch];
        }
    }
}
