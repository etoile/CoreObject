...
- (NSInteger)assignLevelForUUID: (ETUUID *)currentRevision
        greaterThanSiblingLevel: (NSInteger)siblingLevel
{
    ...
    else
    {
        ...
        {
            const NSInteger level = MAX(siblingLevel + 1,
                                        MAX(value + 1, [parentLevel integerValue]));
            levelForUUID[currentRevision] = @(level);
        }
    }
...
- (void)drawLineFromUUID: (ETUUID *)commit1
                  toUUID: (ETUUID *)commit2
              currentRow: (NSUInteger)row
                  inRect: (NSRect)aRect
{
...
- (void)drawRevisionAtIndex: (NSUInteger)index inRect: (NSRect)aRect
{
    ...
    if (isCurrentCommit)
    {
        NSBezierPath *outline = [NSBezierPath bezierPathWithOvalInRect: NSInsetRect(circleRect,
                                                                                    -1,
                                                                                    -1)];
