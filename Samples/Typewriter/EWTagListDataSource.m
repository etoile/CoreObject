...
- (void)reloadData
{
    ...
    [self.outlineView expandItem: nil
                  expandChildren: YES]; // Initially expand all tags - needs to be done before the selection restoration
    ...
    else if ([[[info draggingPasteboard] types] containsObject: EWNoteDragType])
    {
        ...
        [self.owner commitChangesInBlock: ^
        {
            ...
            [tag addObject: noteRootObject];
                          ...
                          withIdentifier: @"tag-note"
                    descriptionArguments: @[tag.name != nil ? tag.name : @"",
                                            notePersistentRoot.name]];
    }
