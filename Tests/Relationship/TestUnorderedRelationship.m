...
- (void)checkPersistentRootsWithExistingAndNewContextInBlock: (void (^)(CHECK_BLOCK_ARGS))block
{
    [self checkPersistentRootWithExistingAndNewContext: group1.persistentRoot
         ...
         BOOL isNewContext)
       {
           ...
           OutlineItem *testItem1 = [testCtx persistentRootForUUID: item1.persistentRoot.UUID].rootObject;
           OutlineItem *testItem2 =  [testCtx persistentRootForUUID: item2.persistentRoot.UUID].rootObject;
           OutlineItem *testOtherItem1 = [testItem1.persistentRoot branchForUUID: otherItem1.branch.UUID].rootObject;
           UnorderedGroupNoOpposite *testOtherGroup1 = [testGroup1.persistentRoot branchForUUID: otherGroup1.branch.UUID].rootObject;

           UnorderedGroupNoOpposite *testCurrentGroup1 = testPersistentRoot.currentBranch.rootObject;
           OutlineItem *testCurrentItem1 = [testCtx persistentRootForUUID: item1.persistentRoot.UUID].currentBranch.rootObject;
           OutlineItem *testCurrentItem2 = [testCtx persistentRootForUUID: item2.persistentRoot.UUID].currentBranch.rootObject;
           OutlineItem *testCurrentOtherItem1 = [testCtx persistentRootForUUID: otherItem1.persistentRoot.UUID].currentBranch.rootObject;
           UnorderedGroupNoOpposite *testCurrentOtherGroup1 = [testCtx persistentRootForUUID: otherGroup1.persistentRoot.UUID].currentBranch.rootObject;

           UKObjectsSame(testCurrentGroup1,
                         testCurrentOtherGroup1);
           UKObjectsSame(testCurrentItem1,
                         testCurrentOtherItem1);
...
- (void)testTargetPersistentRootDeletion
{
    ...
    [self checkPersistentRootsWithExistingAndNewContextInBlock:
        ^(CHECK_BLOCK_ARGS)
        {
...
- (void)testTargetPersistentRootUndeletion
{
    ...
    [self checkPersistentRootsWithExistingAndNewContextInBlock:
        ^(CHECK_BLOCK_ARGS)
        {
            ...
            // Check that the relationship cache knows the inverse relationship,
            // even though it is not used in the metamodel (non-public API)
            UKObjectsEqual(S(testGroup1, testCurrentGroup1, testOtherGroup1),
                           testItem1.referringObjects);
...
- (void)testTargetPersistentRootLazyLoading
{
    ...
    OutlineItem *item1ctx2 = [[group1ctx2.contents objectsPassingTest: ^(COObject *obj, BOOL *stop)
                                                                       {
                                                                           return [obj.UUID isEqual: item1.UUID];
                                                                       }] anyObject];
    OutlineItem *item2ctx2 = [[group1ctx2.contents objectsPassingTest: ^(COObject *obj, BOOL *stop)
                                                                       {
...
- (void)testTargetBranchLazyLoading
{
    ...
    UKObjectsEqual(S(otherItem1Path, item2Path),
                   [[group1ctx2 serializableValueForStorageKey: @"contents"] allReferences]);

    UKObjectsEqual(A(group1ctx2),
                   [[ctx2.deadRelationshipCache referringObjectsForPath: otherItem1Path] allObjects]);
    UKObjectsEqual(A(group1ctx2),
                   [[ctx2.deadRelationshipCache referringObjectsForPath: item2Path] allObjects]);
    ...
    UKObjectsEqual(S(otherItem1ctx2, item2Path),
                   [[group1ctx2 serializableValueForStorageKey: @"contents"] allReferences]);
    UKFalse(ctx2.hasChanges);
...
- (void)testSourcePersistentRootLazyLoadingReverseOrder
{
    ...
    UKObjectsEqual(S([COPath pathWithPersistentRoot: item1uuid],
                     [COPath pathWithPersistentRoot: item2uuid]),
                   [[group1ctx2 serializableValueForStorageKey: @"contents"] allReferences]);
