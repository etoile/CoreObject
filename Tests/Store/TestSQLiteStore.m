...
- (void)testDeleteBranchA
{
    ...
    UKObjectsEqual(S(branchAUUID, branchBUUID, initialBranchUUID),
                   [store persistentRootInfoForUUID: prootUUID].branchUUIDs);
...
//    [store commitStoreTransaction: txn];
    ...
    UKObjectsEqual(S(branchBUUID, initialBranchUUID),
                   [store persistentRootInfoForUUID: prootUUID].branchUUIDs);
}
...
- (void)testBranchMetadata
{
    ...
    UKObjectsEqual(D(@"hello world", @"msg"),
                   [store persistentRootInfoForUUID: prootUUID].currentBranchInfo.metadata);
...
- (void)testPersistentRootMetadata
{
    ...
    UKObjectsEqual(D(@"hello world", @"msg"),
                   [store persistentRootInfoForUUID: prootUUID].metadata);
...
{
    UKObjectsEqual(initialBranchUUID,
                   [store persistentRootInfoForUUID: prootUUID].currentBranchUUID);
...
- (void)testAttachmentsBasic
{
    ...
    UKObjectsEqual(fakeAttachment1,
                   [NSString stringWithContentsOfURL: [store URLForAttachmentID: hash1]
                                            encoding: NSUTF8StringEncoding
                                               error: NULL]);

    UKObjectsEqual(fakeAttachment2,
                   [NSString stringWithContentsOfURL: [store URLForAttachmentID: hash2]
                                            encoding: NSUTF8StringEncoding
                                               error: NULL]);
...
- (void)testAttachmentFromData
{
    ...
    UKObjectsEqual(fakeAttachment,
                   [NSString stringWithContentsOfURL: [store URLForAttachmentID: hash]
                                            encoding: NSUTF8StringEncoding
                                               error: NULL]);
...
- (void)testAttachmentsGCDoesNotCollectReferenced
{
    ...
    [[tree itemForUUID: childUUID1] setValue: hash
                                forAttribute: @"attachment"
                                        type: kCOTypeAttachment];
...
- (void)testRevisionGCDoesNotCollectReferenced
{
    ...
    UKObjectsEqual(tree,
                   [store itemGraphForRevisionUUID: referencedRevision persistentRoot: prootUUID]);
}
...
- (void)testRevisionGCCollectsUnReferenced
{
    ...
    UKObjectsEqual(tree,
                   [store itemGraphForRevisionUUID: unreferencedRevision
...
{
    CORevisionInfo *info = [store revisionInfoForRevisionUUID: initialRevisionUUID
                                           persistentRootUUID: prootUUID];
...
- (void)testPersistentRootCopies
{
    ...
    UKObjectsEqual(copy.currentBranchInfo.currentRevisionUUID,
                   [store persistentRootInfoForUUID: copy.UUID].currentBranchInfo.currentRevisionUUID);
...
- (void)testPersistentRootInsertion
{
    ...
    UKObjectsEqual(initialBranchUUID,
                   [store persistentRootInfoForUUID: prootUUID].currentBranchUUID);
    UKObjectsEqual(cheapCopyBranchUUID,
                   [store persistentRootInfoForUUID: cheapCopyUUID].currentBranchUUID);
}
...
- (void)testReopenStore
{
    ...
    UKObjectsEqual([self makeInitialItemTree],
                   [store2 itemGraphForRevisionUUID: currentRevisionUUID
                                     persistentRoot: prootUUID]);
    UKTrue(COItemGraphEqualToItemGraph([self makeBranchAItemTreeAtIndex: BRANCH_LENGTH - 1],
                                       [store2 itemGraphForRevisionUUID: branchARevisionUUID
                                                         persistentRoot: prootUUID]));
    UKTrue(COItemGraphEqualToItemGraph([self makeBranchBItemTreeAtIndex: BRANCH_LENGTH - 1],
                                       [store2 itemGraphForRevisionUUID: branchBRevisionUUID
                                                         persistentRoot: prootUUID]));
}
...
- (void)testEmptyCommitWithNoChanges
{
    ...
    UKObjectsEqual([self makeInitialItemTree],
                   [store itemGraphForRevisionUUID: revid persistentRoot: prootUUID]);
}
...
- (void)testChangeRootObjectUUIDDisallowed
{
    ...
    COItem *newRootItem = [[COItem alloc] initWithUUID: newRootUUID
                                    typesForAttributes: @{}
                                   valuesForAttributes: @{}];
    COItemGraph *graph = [[COItemGraph alloc] initWithItems: @[newRootItem]
                                               rootItemUUID: newRootUUID];
...
- (void)testAccessItemGraphAfterPersistentRootDeletion
{
    ...
    UKTrue(COItemGraphEqualToItemGraph([self itemTreeWithChildNameChange: @"newRevisionUUID"],
                                       [store itemGraphForRevisionUUID: newRevisionUUID
                                                        persistentRoot: cheapCopyUUID1]));
...

    {
        ...
        cheapCopy1TransactionID = [txn setOldTransactionID: cheapCopy1TransactionID
                                         forPersistentRoot: cheapCopyUUID1];
        cheapCopy1TransactionID = [txn setOldTransactionID: cheapCopy2.transactionID
                                         forPersistentRoot: cheapCopyUUID2];
    ...
    UKTrue(COItemGraphEqualToItemGraph([self itemTreeWithChildNameChange: @"newRevisionUUID"],
                                       [store2 itemGraphForRevisionUUID: newRevisionUUID
                                                         persistentRoot: cheapCopyUUID1]));
    UKTrue(COItemGraphEqualToItemGraph([self itemTreeWithChildNameChange: @"newRevisionUUID"],
                                       [store2 itemGraphForRevisionUUID: newRevisionUUID
                                                         persistentRoot: cheapCopyUUID2]));
