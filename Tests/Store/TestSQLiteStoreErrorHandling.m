...
- (void)testDatabasesBecomingReadonly
{
    ...
    for (NSString *filename in [[NSFileManager defaultManager] contentsOfDirectoryAtPath: dir
                                                                                   error: NULL])
    {
#ifdef GNUSTEP
        assert([[NSFileManager defaultManager] changeFileAttributes: READONLY_SEARCHABLE_DIRECTORY_ATTRIBUTES
    ...
    @autoreleasepool
    {
        ...
        UKObjectsEqual([self makeInitialItemGraph],
                       [store itemGraphForRevisionUUID: info.currentRevisionUUID
                                        persistentRoot: info.UUID]);
    }
