...
- (void)testNoExistingCommitTrack
{
    ...
    [self checkBranchWithExistingAndNewContext: originalBranch
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(testBranch.currentRevision,
                          [testProot.rootObject revision]);
       }];
}
...
- (void)testBranchObjectGraphs
{
    ...
    UKObjectsNotSame([branchA.objectGraphContext rootObject],
                     [branchB.objectGraphContext rootObject]);
    ...
    [branchBroot insertObject: childB
...
- (void)testBranchMetadata
{
...

    {
        COEditingContext *ctx2 = [COEditingContext contextWithURL: store.URL];
        UKObjectsEqual(@{},
                       [ctx2 persistentRootForUUID: persistentRoot.UUID].currentBranch.metadata);
    }
...
- (void)testBranchMetadataOnBranchFirstCommit
{
    ...
    [self checkBranchWithExistingAndNewContext: branch2
       ...
       ^(COEditingContext *testCtx,
         ...
         BOOL isNewContext)
       {
           UKObjectsEqual(D(@"test", kCOBranchLabel),
                          testBranch.metadata);
       }];
}
...
- (void)testSimpleMerge
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: persistentRoot
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(initialRev,
                          secondBranchRev.parentRevision);

           UKObjectsNotEqual(initialBranchRev,
                             secondBranchRev);
           UKObjectsNotEqual(initialBranchRev,
                             initialRev);
           UKObjectsNotEqual(initialRev,
                             secondBranchRev);

           // Check for the proper contents

           UKObjectsEqual(@"1",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: initialBranchRev] rootObject] label]);
           UKObjectsEqual(@"0",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: initialBranchRev] loadedObjectForUUID: childObj.UUID] label]);

           UKObjectsEqual(@"0",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: secondBranchRev] rootObject] label]);
           UKObjectsEqual(@"2",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: secondBranchRev] loadedObjectForUUID: childObj.UUID] label]);

           UKObjectsEqual(@"0",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: initialRev] rootObject] label]);
           UKObjectsEqual(@"0",
                          [(OutlineItem *)[[testProot objectGraphContextForPreviewingRevision: initialRev] loadedObjectForUUID: childObj.UUID] label]);
       }];
