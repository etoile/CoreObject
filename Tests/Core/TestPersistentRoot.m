...
- (void)testBranchCreation
{
    ...
    UKIntsEqual(1,
                [store persistentRootInfoForUUID: persistentRoot.UUID].branchForUUID.allKeys.count);
...
- (void)testBranchSwitchCommitted
{
    ...
    [photo1root insertObject: childA
    ...
    [self checkPersistentRootWithExistingAndNewContext: photo1
                                               inBlock:
       ^(COEditingContext *testCtx,
         COPersistentRoot *testPhoto1,
         COBranch *testBranch,
         BOOL isNewContext)
       {
           UKObjectsEqual(branchB.UUID,
                          testPhoto1.currentBranch.UUID);
           UKObjectsEqual(A(@"childB"),
                          [testPhoto1.rootObject valueForKeyPath: @"contents.label"]);
       }];
}
...
- (void)testBranchSwitchPersistent
{
    ...
    [self checkBranchWithExistingAndNewContext: secondBranch
       ...
       ^(COEditingContext *testCtx,
         COPersistentRoot *testProot,
         COBranch *testSecondBranch,
         BOOL isNewContext)
       {
           UKObjectsEqual(testSecondBranch,
                          testProot.currentBranch);
           UKObjectsEqual(@"hello2",
                          [testProot.rootObject label]);
           UKObjectsEqual(@"hello",
                          [[[testProot branchForUUID: originalBranch.UUID] rootObject] label]);
       }];
}
...
- (void)testCheapCopyCreation
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: copyRoot
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsNotEqual(originalBranch.UUID,
                             testBranch.UUID);
           UKObjectsNotEqual(persistentRoot.UUID,
                             testProot.UUID);

           UKObjectsEqual(originalBranch.UUID,
                          testBranch.parentBranch.UUID);
           UKObjectsEqual(persistentRoot.UUID,
                          testProot.parentPersistentRoot.UUID);

           UKObjectsEqual(rev1,
                          testBranch.initialRevision);
           UKObjectsEqual(rev1,
                          testBranch.currentRevision);
           UKObjectsEqual(rev1,
                          testBranch.headRevision);
...
- (void)testCheapCopyCreationWithEdit
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: copyRoot
       ...
       ^(COEditingContext *testCtx,
         ...
         BOOL isNewContext)
       {
           UKObjectsNotEqual(r1,
                             testBranch.initialRevision);
           UKObjectsNotEqual(r1,
                             testProot.currentRevision);
           UKObjectsEqual(@"a change",
                          [testProot.rootObject label]);
       }];
}
...
- (void)testDeleteCommittedBranch
{
    ...
    [self checkBranchWithExistingAndNewContext: branch
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(S(testOriginalBranch),
                          testProot.branches);
           UKTrue([testProot.branchesPendingDeletion isEmpty]);
           UKObjectsEqual(S(testBranch),
                          testProot.deletedBranches);
...
- (void)testUndeleteCommittedBranch
{
    ...
    [self checkBranchWithExistingAndNewContext: branch
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(S(testBranch, testOriginalBranch),
                          testProot.branches);
...
- (void)testMixingObjectsAmongObjectGraphsBelongingToSinglePersistentRoot
{
    ...
    [altBranch.rootObject insertObject: child
                               atIndex: ETUndeterminedIndex
                                  hint: nil
                           forProperty: @"contents"];

    UKRaisesException([originalBranch.rootObject insertObject: child
                                                      atIndex: ETUndeterminedIndex
                                                         hint: nil
                                                  forProperty: @"contents"]);
}
...
- (void)testBranchesAccessors
{
    ...
    UKObjectsEqual(S(current, regular, pendingInsertion, pendingUndeletion),
                   persistentRoot.branches);
    ...
    [self checkPersistentRootWithExistingAndNewContext: persistentRoot
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(S(testCurrent,
                            testRegular,
                            testPendingInsertion,
                            testPendingUndeletion),
                          testPersistentRoot.branches);
           UKObjectsEqual(S(testDeletedOnDisk,
                            testPendingDeletion),
                          testPersistentRoot.deletedBranches);
           UKObjectsEqual([NSSet set],
                          testPersistentRoot.branchesPendingInsertion);
           UKObjectsEqual([NSSet set],
                          testPersistentRoot.branchesPendingDeletion);
           UKObjectsEqual([NSSet set],
                          testPersistentRoot.branchesPendingUndeletion);
       }];
}
