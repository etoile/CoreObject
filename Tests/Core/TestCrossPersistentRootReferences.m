...
- (void)testBasic
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: library
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual([photo1.rootObject UUID],
                          [photo1ctx2.rootObject UUID]);
           UKObjectsEqual([photo2.rootObject UUID],
                          [photo2ctx2.rootObject UUID]);
           UKObjectsEqual(@"photo1",
                          [photo1ctx2.rootObject label]);
           UKObjectsEqual(@"photo2",
                          [photo2ctx2.rootObject label]);
       }];
}
...
- (void)testBranchSwitch
{
    ...
    UKObjectsEqual(S(@"photo1, branch A"),
                   [library1.rootObject valueForKeyPath: @"contents.label"]);
    UKObjectsEqual(S(A(@"childA")),
                   [library1.rootObject valueForKeyPath: @"contents.contents.label"]);
    ...
    UKObjectsEqual(S(@"photo1, branch A", @"photo1, branch B"),
                   [library1.rootObject valueForKeyPath: @"contents.label"]);
    UKObjectsEqual(S(A(@"childA"), A(@"childB")),
                   [library1.rootObject valueForKeyPath: @"contents.contents.label"]);
    ...
    [self checkPersistentRootWithExistingAndNewContext: library1
                                               inBlock:
       ^(COEditingContext *ctx2,
         COPersistentRoot *library1ctx2,
         COBranch *testBranch,
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(branchB.UUID,
                          photo1ctx2.currentBranch.UUID);
           UKObjectsEqual(A(@"childB"),
                          [photo1ctx2.rootObject valueForKeyPath: @"contents.label"]);

           // Actual test of cross-persistent-root references

           UKObjectsEqual(S(@"photo1, branch A",
                            @"photo1, branch B"),
                          [library1ctx2.rootObject valueForKeyPath: @"contents.label"]);
           UKObjectsEqual(S(A(@"childA"), A(@"childB")),
                          [library1ctx2.rootObject valueForKeyPath: @"contents.contents.label"]);
       }];
}
...
- (void)testPersonAndPersonGroupBranch
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: group
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(S(testGroupRoot),
                          testJohnRoot.parentGroups);
           UKObjectsEqual(S(testGroupRoot),
                          testLucyRoot.parentGroups);
       }];
    ...
    [self checkBranchWithExistingAndNewContext: groupA
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsEqual(S(testProot.rootObject),
                          testJohnRoot.parentGroups);
           UKObjectsEqual(S(testProot.rootObject),
                          testLucyRoot.parentGroups);

           UKObjectsNotEqual(S(testBranchARoot),
                             testJohnRoot.parentGroups);
           UKObjectsNotEqual(S(testBranchARoot),
                             testLucyRoot.parentGroups);
       }];
    ...
    [self checkBranchWithExistingAndNewContext: groupB
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsNotEqual(S(testBranchBRoot),
                             testJohnRoot.parentGroups);
           UKObjectsNotEqual(S(testBranchBRoot),
                             testLucyRoot.parentGroups);
       }];
    ...
    [self checkBranchWithExistingAndNewContext: lucyA
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsSame(testGroup,
                         [testLucyARoot.parentGroups anyObject]);
    ...
    [self checkBranchWithExistingAndNewContext: lucyB
         ...
         BOOL isNewContext)
       {
           ...
           UKObjectsSame(testGroup,
                         [testLucyBRoot.parentGroups anyObject]);
...
- (void)testBranchDeletion
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: library1
       ...
       ^(COEditingContext *testCtx,
         ...
         BOOL isNewContext)
       {
           UKObjectsEqual(S(@"photo1, branch B"),
                          [[[testProot.rootObject contents] mappedCollection] label]);
       }];
...
- (void)testMultipleRelationshipsPerObject
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: tag1
       ...
       ^(COEditingContext *testCtx,
         ...
         BOOL isNewContext)
       {
           UKObjectsEqual(S([photo1.rootObject UUID],
                            [photo2.rootObject UUID]),
                          [[[testProot.rootObject contents] mappedCollection] UUID]);
       }];
    ...
    [self checkPersistentRootWithExistingAndNewContext: tag1
       ...
       ^(COEditingContext *testCtx,
         ...
         BOOL isNewContext)
       {
           UKObjectsEqual(S([photo2.rootObject UUID]),
                          [[[testProot.rootObject contents] mappedCollection] UUID]);
       }];
}
...
- (void)testPersistentRootDeletion
{
    ...
    [library1.rootObject insertObject: photo1.rootObject
...
- (void)testPersistentRootUndeletion
{
    ...
    [self checkPersistentRootWithExistingAndNewContext: library1
         ...
         BOOL isNewContext)
       {
           ...
           UKFalse(library1ctx2.objectGraphContext.hasChanges);
           UKObjectsEqual(S(@"photo1", @"photo2"),
                          [library1ctx2.rootObject valueForKeyPath: @"contents.label"]);
       }];
}
