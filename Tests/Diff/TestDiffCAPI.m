...
- (void)checkEdit: (diffedit_t)anEdit
   isInsertAtLocA: (int)locA
         fromLocB: (int)locB
           length: (int)lenB
{
    UKIntsEqual(locA, anEdit.range_in_a.location);
...
- (void)testMixed
{
    ...
    diffresult_t *diff = diff_arrays(strlen(array1),
                                     strlen(array2),
                                     arraycomparefn,
                                     array1,
                                     array2);
    ...
    [self checkEdit: diff_edit_at_index(diff, 0)
     isCopyFromLocA: 0
             length: 1
             toLocB: 0];   // copy 'a'
    [self checkEdit: diff_edit_at_index(diff, 1)
   isDeleteFromLocA: 1
             length: 1];          // delete 'b'
    [self checkEdit: diff_edit_at_index(diff, 2)
     isCopyFromLocA: 2
             length: 1
             toLocB: 1];   // copy 'c'
    [self checkEdit: diff_edit_at_index(diff, 3)
     isInsertAtLocA: 3
           fromLocB: 2
             length: 2]; // insert 'hi'
    [self checkEdit: diff_edit_at_index(diff, 4)
     isCopyFromLocA: 3
             length: 1
             toLocB: 4];   // copy 'd'
    [self checkEdit: diff_edit_at_index(diff, 5)
   isModifyFromLocA: 4
             length: 2
             toLocB: 5
             length: 3]; // modify 'ef' to 'xyz'
    [self checkEdit: diff_edit_at_index(diff, 6)
     isCopyFromLocA: 6
             length: 1
             toLocB: 8];   // copy 'g'
