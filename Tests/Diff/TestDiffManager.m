...
- (void)testAttributedStringAndItemGraphMerge
{
    ...
    COObjectGraphContext *branchA = [COObjectGraphContext new];
    {
        ...
        [self appendString: @"this is a big test"
                  htmlCode: @"i"
    ...
    CODiffManager *diffBaseBranchA = [CODiffManager diffItemGraph: base
                                                    withItemGraph: branchA
                                       modelDescriptionRepository: base.modelDescriptionRepository
                                                 sourceIdentifier: @"branchA"];
    CODiffManager *diffBaseBranchB = [CODiffManager diffItemGraph: base
                                                    withItemGraph: branchB
                                       modelDescriptionRepository: base.modelDescriptionRepository
                                                 sourceIdentifier: @"branchB"];
...

    {
       ...
       withLongestEffectiveRange: NSMakeRange(0, 5)
              inAttributedString: w1];
        // This may not be exactly what we want. the bold "n't" did not pick up the italics
        [self checkFontHasTraits: NSFontItalicTrait | NSFontBoldTrait
       withLongestEffectiveRange: NSMakeRange(5, 2)
              inAttributedString: w1];
        [self checkFontHasTraits: NSFontBoldTrait
       withLongestEffectiveRange: NSMakeRange(7, 3)
              inAttributedString: w1];
        [self checkFontHasTraits: NSFontItalicTrait
       withLongestEffectiveRange: NSMakeRange(10, 11)
              inAttributedString: w1];
