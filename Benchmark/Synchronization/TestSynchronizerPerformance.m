...
- (void)testAttributedStringRebasePerformance
{
    ...
    for (NSUInteger i = 0; i < charactersToInsert.length; i++)
    {
        [clientWrapper replaceCharactersInRange: NSMakeRange(i, 0)
                                     withString: [charactersToInsert substringWithRange: NSMakeRange(
                                         i,
                                         1)]];
    ...
    UKTrue(([[NSString stringWithFormat: @"%@%@%@",
                                         stringToInsert,
                                         [charactersToInsert substringToIndex: 1],
                                         baseString] isEqualToString: serverWrapper.string]
    ...
    UKTrue(([[NSString stringWithFormat: @"%@%@%@",
                                         stringToInsert,
                                         charactersToInsert,
                                         baseString] isEqualToString: clientWrapper.string]
            || [[NSString stringWithFormat: @"%@%@%@",
                                            charactersToInsert,
                                            stringToInsert,
                                            baseString] isEqualToString: clientWrapper.string]));
